...
class Discriminator(nn.Module):
    def __init__(self, img_channels, filters=[64, 128, 256, 512]):
        ...
        self.initial = nn.Sequential(
            nn_utils.spectral_norm(nn.Conv2d(img_channels, filters[0], kernel_size=4, stride=2, padding=1, padding_mode="reflect")),
            nn.LeakyReLU(0.2, inplace=True)
        )
        ...
        self.last = nn_utils.spectral_norm(nn.Conv2d(filters[3], 1, kernel_size=4, stride=1, padding=1, padding_mode="reflect"))

    def forward(self, x):
        x = self.initial(x)
        x = self.discblocks(x)
        out = torch.sigmoid(self.last(x))
        return out
